...
//
//  Created by David Eriksson on 8/25/15.
//  Copyright (c) 2015 David Eriksson. All rights reserved.
//

#ifndef Surrogate_Optimization_optimizer_lsmrbf_h
#define Surrogate_Optimization_optimizer_lsmrbf_h
...
namespace sot {
    ...
    class OptimizerLMSRBF : public Optimizer {
    private:
        double sigma_max = 0.2, sigma_min = 0.005;
        int failtol, succtol;
        bool do_restart;
    public:
        OptimizerLMSRBF(Problem *data, ExpDesign *exp_des, int maxeval) {
            this->data = data;
        ...
        OptimizerLMSRBF(Problem *data, ExpDesign *exp_des, int maxeval, bool do_restart) : OptimizerLMSRBF(data, exp_des, maxeval) {
        ...
        start:
            ...
            while (numeval < maxeval) {
                ...
                vec newx = AllDim<Surface>(rbf, rbf.get_centers(), data->lbound(), data->rbound(), 
