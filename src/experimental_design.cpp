...
#include "../include/sot_bits/experimental_design.h"
#include "../include/sot_bits/common.h"

///////////////////////////////////// Symmetric LHS //////////////////////////////////////

namespace sot {
    
    mat SymmetricLatinHypercube::create_design() const {
    ...
    SymmetricLatinHypercube::SymmetricLatinHypercube(int num_points, int dim) {
        this->num_points = num_points;
        this->dim = dim;
        assert(num_points >= 2 * dim);
    }
    
    mat SymmetricLatinHypercube::generate_points() const {
    ...
    LatinHypercube::LatinHypercube(int num_points, int dim) {
        this->num_points = num_points;
        this->dim = dim;
        assert(num_points >= dim);
    }
    
    mat LatinHypercube::generate_points() const {
    ...
    TwoFactorial::TwoFactorial(int dim) {
        this->num_points = pow(2, dim);
        this->dim = dim;
        assert(dim <= 15);
    }
    
    mat TwoFactorial::generate_points() const {
    ...
    CornersMid::CornersMid(int dim) {
        this->num_points = 1 + pow(2, dim);
        this->dim = dim;
        assert(dim <= 15);
    }
    
    mat CornersMid::generate_points() const {
        mat xsample = arma::zeros<mat>(dim, num_points);
