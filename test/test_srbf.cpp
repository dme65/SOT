...
int test_srbf() {
    ...
    std::shared_ptr<Surrogate> rbf(std::make_shared<TPSRBF>(maxeval, dim, data->lBounds(), data->uBounds()));
    std::shared_ptr<Sampling> dycors(std::make_shared<SRBF<>>(data, rbf, 100*dim, maxeval - slhd->numPoints()));
